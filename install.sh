...
adb $1 shell input text "busybox\ telnetd\ -Fb\ 127.0.0.1:8023\;\ exit\ 0" &&
adb $1 shell input keyevent 66 &&
adb $1 forward tcp:8023 tcp:8023 > /dev/null

until [ "$(echo exit | nc localhost 8023 | xargs)" ]; do
	sleep 0.1 && ((i++))
	if [ $i -ge 100 ]; then
...
quit () {
	echo "killall busybox || logout" | nc localhost 8023 > /dev/null
}
trap quit EXIT
...
cmd () {
	...
	OUT=$(echo 'bash -c '\'"$1"\'' || echo "An error occurred (exit code $?)"; logout' |
		nc localhost 8023 | sed -u "$sed" | tee /dev/tty | tail -n1)
...
echo $'\n[*] Installing yt-dlp'
cmd "apt -y install python && pip --disable-pip-version-check install yt-dlp"
